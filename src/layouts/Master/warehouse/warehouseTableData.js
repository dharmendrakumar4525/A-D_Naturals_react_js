...
export default function data() {
  ...
  const handleDelete = async (sellerId) => {
    try {
      await axios.delete(`${environment.api_path}/warehouse/${sellerId}`);
      setRowData((prevData) => prevData.filter((warehouse) => warehouse._id !== sellerId));
    } catch (error) {
  ...
  return {
    ...
    rows: rowData.map((warehouse) => ({
      name: <Author name={warehouse.warehouse_name} />,
      action: (
        <>
