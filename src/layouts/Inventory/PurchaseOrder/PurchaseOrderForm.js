      ...
      <Box sx={style}>
        ...
        >
          <MuiAlert
            ...
            variant="filled"
            onClose={() => setOpenSnackbar(false)}
            severity={submitError === "Purchase Order Created Successfully" ? "success" : "error"}
          >
            {submitError}
          </MuiAlert>
        </Snackbar>
        ...
        <TableBody>
          {warehouseArray.map((warehouse) => (
            <TableRow key={warehouse.warehouse}>
              <TableCell sx={{ fontSize: 14 }}>{getWarehouseName(warehouse.warehouse)}</TableCell>
              <TableCell>
                <TextField
                  fullWidth
                  label="Qty"
